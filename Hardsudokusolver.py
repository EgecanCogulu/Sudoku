# -*- coding: utf-8 -*-
"""
Created on Fri Feb 14 12:15:19 2020
...
#Input coordinates row,col, it will output coordinates of the corresponding big squares.
...
def guesser(grid,arrays_dict,guesses):
    guesses=[]
    for number in range(1,10):
        for row in range(9):
            if np.count_nonzero(arrays_dict[number][row])==2:
                location=np.where(arrays_dict[number][row] == True)[0][0]
                grid[row][location]=number
                np.append(guesses,[number,row,location])
                return (grid,arrays_dict,guesses)
        for col in range(9):
            if np.count_nonzero(arrays_dict[number][:,col])==2:
                location=np.where(arrays_dict[number][:,col] == True)[0][0]
                grid[location][col]=number
                return (grid,arrays_dict,guesses)
        ...
        for bigsquarenumber in range(9):
            (bigsquare,coor)=bigsquare_arrays(arrays_dict[number],bigsquarenumber)
            if np.count_nonzero(bigsquare)==2:
                location=np.where(bigsquare==True)
                grid[location[0][0]+coor[0]][location[1][0]+coor[1]]=number
                return (grid,arrays_dict,guesses)
